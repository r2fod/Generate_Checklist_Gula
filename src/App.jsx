...
function calcCristaleria(pax, h, dobleCopa, tieneBrindisCava, llevaEntrante) {
  ...
  return {
    agua:   fmt(agua, BATEA.agua),
    cubata: fmt(cubata, BATEA.cubata),
    vino:   fmt(vino, BATEA.vino),
    cava:   fmt(cavaCopas, BATEA.cava),
...
// ─── GENERADOR DEL CHECKLIST COMPLETO ────────────────────────────────────────
function buildChecklist(evtKey, pax, horasCoctel, horasCopas, ninos, opts) {
  const {
    ...
    tieneFrituras, llevaEntrante, llevaCafe, llevaArmarioCaliente,
  } = opts;
  ...
  const mobItems = [
    ...
    ["Sacacorchos", "2"],
    ["Abridores cerveza", "2"],
    ["Bandeja camarero", String(Math.max(2, Math.ceil(pax / 20)))],
  ...
  cats.push({ nombre: "Mobiliario, sala y decoración", items: mobItems });
  ...
  if (llevaPaella) {
    ...
    cocinaItems.push(
      [`Paella ${p.talla}`, String(p.n)],
      ["Difusores", String(p.n)],
      ["Trípode", String(p.n)],
      ["Paravientos", String(p.n)],
    );
  }
  ...
  if (tieneFrituras) cocinaItems.push(
    ["Sartén Parisiene (frituras)", "1"],
    ["Difusor extra (frituras)", "1"],
    ["Trípode extra (frituras)", "1"],
    ["Espumadera grande", "2"],
  );
  if (tipoBBQ !== "no lleva") {
    const nBBQ = Math.max(1, Math.ceil(pax / 60));
    cocinaItems.push(
      [`Barbacoa ${tipoBBQ}`, String(nBBQ)],
      ["Carbón", String(Math.max(2, Math.ceil(pax / 30)))],
      ["Leña", "1"],
      ["Pastillas de encender", "1"],
    );
  }
  cats.push({ nombre: "Cocina y fuego", items: cocinaItems });

  // 4. MENAJE Y UTENSILIOS
  cats.push({ nombre: "Menaje y utensilios", items: [
    ["Maletín de cuchillos", "1"],
    ["Tablas de corte", "2"],
    ["Ollas (mediana y grande)", "1"],
    ["Sartenes", "1"],
    ["Colador", "1"],
    ["Boles metálicos", "4"],
  ...
  cats.push({ nombre: "Cristalería", items: crItems });

  // 6. MANTELERÍA Y TEXTILES
  const totalMesas = calcMesasTotal(evtKey, pax);
  cats.push({ nombre: "Mantelería y textiles", items: [
    ["Manteles", String(totalMesas + 2)],
    ["Delantales cocina y sala", "5"],
  ...
  cats.push({ nombre: "Vajilla", items: vaItems });

  // 8. SERVICIO Y LIMPIEZA
  cats.push({ nombre: "Servicio y limpieza", items: [
    ["Fairy", "1"],
    ["Estropajo", "1"],
    ["Papel plata", "1"],
    ["Film", "1"],
    ["Bayetas y trapos de horno", "4"],
    ["Papel Chemine", "2"],
    ["Bolsas de basura", "10"],
    ["Ceniceros", "—"],
  ]});
  ...
  // 9. CAFÉ (opcional)
  if (llevaCafe) {
    const capsPorPax = Math.round(totalPax * 0.7);
    cats.push({ nombre: "Café", items: [
      ["Cafetera grande / Nespresso", "1"],
      ["Cápsulas café", String(capsPorPax)],
      ["Tazas café solo", String(Math.round(totalPax * 0.4))],
      ["Tazas café con leche", String(Math.round(totalPax * 0.25))],
      ["Infusiones", String(Math.round(totalPax * 0.2))],
      ["Azucarillos y edulcorantes", String(Math.round(totalPax * 1.1))],
      ["Leches variadas (entera/sin lactosa/avena)", String(Math.max(3, Math.ceil(totalPax / 20)))],
    ]});
  }
  ...
  cats.push({ nombre: "Bebidas frías", items: [
    ...
    ["Cava", `${bebidas.cava} botellas`],
    ["Agua 1,5L", `${bebidas.agua15} packs`],
    ["Coca-Cola normal", String(bebidas.cocaNormal)],
    ["Coca-Cola Zero", String(bebidas.cocaZero)],
    ["Fanta / Aquarius", String(bebidas.fanta)],
    ["Sprite", String(bebidas.sprite)],
    ["Nestea", String(bebidas.nestea)],
    ["Tónica", `${bebidas.tonica} botellas`],
    ["Agua con gas", String(bebidas.aguaConGas)],
    ["Cerveza 0,0", String(bebidas.cerveza00)],
    ["Cerveza sin gluten", String(bebidas.sinGluten)],
    ["Hielo", `${bebidas.taxisHielo} ${tieneCongelador ? "cajas almacén" : "taxis"}`],
    ...(hayBarra ? [["Redbull", String(bebidas.redbull)]] : []),
  ]});

  // 11. ALCOHOLES Y LICORES
  if (destilados) {
    cats.push({ nombre: "Alcoholes y licores", items: [
      ...
      ["Ron (Bacardí)", String(destilados.ron)],
      ["Ron saborizado (Negrita)", String(destilados.ronBlanco)],
      ["Tequila", String(destilados.tequila)],
      ["Tequila Rosa", String(destilados.tequilaSabor)],
      ["Vodka", String(destilados.vodka)],
      ["Vermut rojo", String(destilados.vermut)],
      ["Mistela", String(destilados.mistela)],
      ["Baileys", String(destilados.baileys)],
      ["Tía María", String(destilados.tiaMaria)],
      ["Limoncello", String(destilados.limoncello)],
      ["Jagger", String(destilados.jagger)],
      ["Peach", String(destilados.peach)],
      ["Crema de orujo", String(destilados.cremaOrujo)],
      ["Cazalla", String(destilados.cazalla)],
      ["Orujo de hierbas", String(destilados.orujoHierbas)],
      ["Otros licores marca blanca (Smirnoff)", String(destilados.marcaBlanca)],
    ]});
  }

  // 12. LOGÍSTICA Y RETORNO
  cats.push({ nombre: "Logística y retorno", items: [
    ["Cajas extra platos sucios", "1"],
    ["Cajas extra cubiertos sucios", "1"],
    ["Caja azul extra", "1"],
    ["Taxis comida", "—"],
  ]});
...
// ─── GENERADOR HTML PARA WORD ─────────────────────────────────────────────────
function generarHTMLWord(evento, evtKey, pax, ninos, horasCoctel, horasCopas, barraCoctel, barraCopas, checklist) {
  const fecha = new Date().toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" });
  const colsSale = ["Concepto", "Cant.", "Sale ✓", "Vuelve ✓", "Roturas"];

  const tablaHTML = (items) => `
    <table border="1" cellpadding="6" cellspacing="0" style="width:100%;border-collapse:collapse;font-size:11pt;">
      <thead>
        <tr style="background:#1f314d;color:white;">
          ${colsSale.map(c => `<th style="text-align:left;padding:6px;">${c}</th>`).join("")}
        </tr>
      </thead>
      <tbody>
        ${items.map(([label, qty], i) => {
  ...
  const secciones = checklist.map(cat => `
    <h3 style="background:#1f314d;color:white;padding:8px 12px;font-size:11pt;margin:18px 0 0 0;text-transform:uppercase;">${cat.nombre}</h3>
    ${tablaHTML(cat.items)}`).join("");
...
<style>
  ...
  h1 { color: #1f314d; font-size: 18pt; margin-bottom: 4px; }
  h2 { color: #444; font-size: 12pt; font-weight: normal; margin-top: 4px; }
  .meta-grid { display: flex; gap: 40px; background: #f3f4f6; padding: 12px 16px; border-radius: 4px; margin: 16px 0; font-size: 10pt; }
...
<body>
<h1>CHECKLIST DE EVENTO</h1>
<h2>${EVENTOS[evtKey]?.label?.toUpperCase()} · ${pax} PAX</h2>
<div class="meta-grid">
  <div><span class="meta-label">Evento</span> ${EVENTOS[evtKey]?.label}</div>
  <div><span class="meta-label">Fecha generación</span> ${fecha}</div>
  <div><span class="meta-label">PAX total</span> ${pax + ninos} (${pax} adultos${ninos > 0 ? ` + ${ninos} niños` : ""})</div>
  <div><span class="meta-label">Barra libre</span> ${barraCoctel ? `Cóctel ${horasCoctel}h` : "—"}${barraCopas ? ` + Copas ${horasCopas}h` : ""}</div>
</div>
${secciones}
<div class="notas">
...
// ─── MODAL VISTA PREVIA ───────────────────────────────────────────────────────
function ModalVistaPrevia({ checklist, evento, evtKey, pax, ninos, onClose }) {
  const fecha = new Date().toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" });
  return (
    <div style={{
      ...
      >
        ...
        <div style={{ background: "#1f314d", color: "white", padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: "1rem" }}>✕ Cerrar</button>
        </div>
        ...
        <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: 20 }}>
          {checklist.map(cat => (
            <div key={cat.nombre}>
              <div style={{ background: "#1f314d", color: "white", padding: "8px 12px", fontWeight: 700, fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                {cat.nombre}
              </div>
                ...
                <thead>
                  <tr style={{ background: "#f3f4f6", color: "#6b7280" }}>
                    {["Concepto", "Cant.", "Sale ✓", "Vuelve ✓", "Roturas"].map(h => (
                      <th key={h} style={{ padding: "6px 10px", textAlign: "left", fontWeight: 600, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {cat.items.map(([label, qty], i) => {
                    ...
                    return (
                      <tr key={i} style={{ background: alq ? "#fdf6e3" : i % 2 === 0 ? "#fff" : "#fafafa", borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <td style={{ padding: "8px 10px", width: 60 }}></td>
                        <td style={{ padding: "8px 10px", width: 60 }}></td>
                        <td style={{ padding: "8px 10px", width: 60 }}></td>
                      </tr>
                    );
...
export default function App() {
  ...
  const [llevaCafe, setLlevaCafe] = useState(false);
  ...
  const opts = {
    ...
    fuerzaTextilTela, tieneFrituras, tipoBandejas,
    tipoBBQ: tipoBBQ.toLowerCase(), tipoHorno: tipoHorno.toLowerCase(),
    llevaEntrante, llevaCafe, llevaArmarioCaliente,
  };
  ...
  const filtered = useMemo(() => {
    ...
    return checklist
      .map(c => ({ ...c, items: c.items.filter(i => i[0].toLowerCase().includes(q)) }))
  ...
  const toggleCategory = (catName) =>
    setOpenCategories(prev => ({ ...prev, [catName]: prev[catName] !== false ? false : true }));

  // ── DESCARGAR WORD ──
  const handleDescargar = () => {
    const html = generarHTMLWord(EVENTOS[evento]?.label, evento, pax, ninos, horasCoctel, horasCopas, barraCoctel, barraCopas, checklist);
  ...
  const handleCompartir = () => {
    ...
    const contenido = `CHECKLIST ${EVENTOS[evento]?.label?.toUpperCase()} · ${pax} pax\n${texto}`;
    navigator.clipboard.writeText(contenido).then(() => {
      setCompartirMsg("¡Copiado al portapapeles! ✓");
  ...
  const SegmentedControl = ({ value, onChange, options, label }) => (
    <div className="segment-group">
      ...
      <div className="segmented-control">
        {options.map(opt => (
          <button key={opt} className={`segment-btn ${value === opt ? "active" : ""}`} onClick={() => onChange(opt)}>
            {opt}
          </button>
        ))}
      </div>
      ...
      <div className="app-wrapper">
        ...
        <header className="app-header animate-entrance">
          ...
          <div className="header-actions">
            <button className="btn btn-outline" onClick={() => setModalAbierto(true)}>Vista previa</button>
            <button className="btn btn-outline" onClick={handleCompartir} title={compartirMsg || "Copiar checklist al portapapeles"}>
              {compartirMsg || "Compartir"}
            </button>
        ...
        <div className="config-card animate-entrance" style={{ animationDelay: "0.1s" }}>
          ...
          <div className="checkbox-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
            {[
              ...
              [llevaCafe,           setLlevaCafe,           "Coffee break / Café",     "cafetera, cápsulas, tazas"],
