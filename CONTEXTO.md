...
```
npm run lint          # oxlint — ~137 warnings de catch(e) son de la casa; ERRORES: 0
...
**335 (cálculos) + 396 (asistente) + 201 (sincronización) + 711 (navegador), 0 fallos.**
...
`firestore.rules` se pega a mano en la consola (sin `firebase.json`, no autodespliega).
Hoy consola y repo coinciden.
...
| `texto.js` | `sinMarcas()` — quita markdown de las respuestas |
